...
#define ACTION_BEGIN_FRAME "beginFrame"
#define ACTION_END_FRAME "endFrame"
#define ACTION_COMMIT "commit"
#define ACTION_READ_PIXELS "readPixels"
...
//    glMatrixRotatedEXT(GL_FRAMEBUFFER, 0, 1, -1, 1); // does not work
    ...
    callScheduedCallback(ACTION_READ_PIXELS, (void *)(long)s->gl.fbo, (void *)(long)pbo, 0);
    ...
    whenGpuCommandsComplete([this, width, height, pbo, pUserBuffer]() mutable {
        ...
        if(long(p.buffer) != 0x1234) {
            ...
            callScheduedCallback(ACTION_COMMIT, nullptr, (void *)(long)pbo, 0);
            glDeleteBuffers(1, &pbo);
            CHECK_GL_ERROR(utils::slog.e)
            delete pUserBuffer;
        }
...
static void dummy(bool presentFrame, void* user) {}
void OpenGLDriver::callScheduedCallback(const char *action, void *a1, void* a2, void *a3) {
    if(sched_callback_) {
        auto& gl = mContext;
        void *ud[5] = {
                (void *)action,
        ...
        backend::PresentCallable callable(dummy, nullptr);
        sched_callback_(callable, ud);
    }
}
void OpenGLDriver::callCompletedCallback(const char *action, void *a1, void* a2, void *a3) {
    if(compl_callback_) {
        auto& gl = mContext;
        void *ud[5] = {
                (void *)action,
