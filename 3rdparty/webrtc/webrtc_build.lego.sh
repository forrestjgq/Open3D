...
build_webrtc_lego() {
    ...
    if [[ $(uname -s) == 'Linux' ]]; then
        tar -czf \
            "$OPEN3D_DIR/webrtc_${WEBRTC_COMMIT_SHORT}_cxx-abi-${GLIBCXX_USE_CXX11_ABI}.tar.gz" \
            webrtc_release
    elif [[ $(uname -s) == 'Darwin' ]]; then
        tar -czf \
            "$OPEN3D_DIR/webrtc_${WEBRTC_COMMIT_SHORT}_macos.tar.gz" \
    ...
    popd # PWD=Open3D
    webrtc_package=$(ls webrtc_*.tar.gz)
    checksum="checksum_${webrtc_package%%.*}.txt"
    cmake -E sha256sum "$webrtc_package" | tee $checksum
    ls -alh "$webrtc_package"
    sudo cp $webrtc_package 3rdparty_downloads/webrtc/
    sum=$(cat $checksum | awk '{print $1')
    sed -i -r "s/jgqsum/${sum}/" 3rdparty/webrtc/webrtc_download.cmake
